<template>
    <div>
        ...
        <v-container class="my-5 teste">
            <v-data-table :headers="headers" :items="cronjobs" :loading="loading" class="mt-10">
                <template v-slot:top>
                    <v-toolbar flat>
                        <h1 class="subheading grey--text">Cron Jobs</h1>
...
<script>
    export default {
      ...
      created(){
          ...
          axios
            .get("http://192.168.1.64:8700/apis/batch/v1beta1/namespaces/"+this.$store.state.nameSpace+"/cronjobs",options)
            .then(response => {
            console.log(response.data.items);
            this.cronjobs = response.data.items;
            this.loading = false
            })
            .catch(error => {
                console.log("Fail");
      ...
      methods:{
        ...
        deleteItem(item){
          ...
          axios.delete('http://192.168.1.64:8700/apis/batch/v1beta1/namespaces/cronjobs/'+item.metadata.name,options)
          .then(response => { 
            this.messageLogin = 'Cron Job eliminado com sucesso';
