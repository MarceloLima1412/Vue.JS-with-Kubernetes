<template>
    <div>
        ...
        <v-container class="my-5 teste">
            <v-data-table :headers="headers" :items="ingresses" :loading="loading" class="mt-10">
                <template v-slot:top>
                    <v-toolbar flat>
                        <h1 class="subheading grey--text">Ingresses</h1>
...
<script>
    ...
    export default {
      ...
      created(){
        ...
        axios
            .get("http://192.168.1.64:8700/apis/networking.k8s.io/v1beta1/namespaces/"+this.$store.state.nameSpace+"/ingresses",options)
            .then(response => {
            console.log(response.data.items);
            this.ingresses = response.data.items;
            this.loading = false
            })
            .catch(error => {
                console.log("Fail");
      ...
      methods:{
        ...
        deleteItem(item){
          ...
          axios.delete('http://192.168.1.64:8700/apis/networking.k8s.io/v1beta1/namespaces/'+this.$store.state.nameSpace+'/ingresses/'+item.metadata.name,options)
          .then(response => { 
            this.messageLogin = 'Ingress eliminado com sucesso';
