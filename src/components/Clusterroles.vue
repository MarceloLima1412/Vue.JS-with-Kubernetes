<template>
    <div>
        ...
        <v-container class="my-5 teste">
            <v-data-table :headers="headers" :items="clusterroles" :loading="loading" class="mt-10">
                <template v-slot:top>
                    <v-toolbar flat>
                        <h1 class="subheading grey--text">Cluster Roles</h1>
...
<script>
    ...
    export default {
      ...
      created(){
            ...
            axios
            .get("http://192.168.1.64:8700/apis/rbac.authorization.k8s.io/v1/clusterroles",options)
            .then(response => {
            console.log(response.data.items);
            this.clusterroles = response.data.items;
            this.loading = false
            console.log("asdasd")
            for (const clusterrole of this.clusterroles){
              ...
              Object.assign(clusterrole, {diascriado: this.dhm(Date.now() - Date.parse(clusterrole.metadata.creationTimestamp))});
            }
            })
            .catch(error => {
                console.log("Fail");
      ...
      methods:{
        ...
        deleteItem(item){
          ...
          axios.delete('http://192.168.1.64:8700/apis/rbac.authorization.k8s.io/v1/clusterroles/'+item.metadata.name,options)
          .then(response => { 
            this.messageLogin = 'Cluster Role eliminado com sucesso';
