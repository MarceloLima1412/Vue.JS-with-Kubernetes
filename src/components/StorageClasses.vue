<template>
    <div>
        ...
        <v-container class="my-5 teste">
            <v-data-table :headers="headers" :items="storageclasses" :loading="loading" class="mt-10">
                <template v-slot:top>
                    <v-toolbar flat>
                        <h1 class="subheading grey--text">Storage Classes</h1>
...
<script>
    ...
    export default {
      ...
      created(){
        ...
        axios
            .get("http://192.168.1.64:8700/apis/storage.k8s.io/v1/storageclasses",options)
            .then(response => {
            console.log("response.data.items");
            console.log(response.data.items);
            this.storageclasses = response.data.items;
            this.loading = false
            })
            .catch(error => {
                console.log("Fail");
      ...
      methods:{
        ...
        deleteItem(item){
          ...
          axios.delete('http://192.168.1.64:8700/apis/storage.k8s.io/v1/storageclasses/'+item.metadata.name,body,options)
          .then(response => { 
            this.messageLogin = 'Storage Class eliminado com sucesso';
