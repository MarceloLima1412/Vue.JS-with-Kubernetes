<template>
    <div>
        ...
        <v-container class="my-5 teste">
            <v-data-table :headers="headers" :items="persistentvolumes" :loading="loading" class="mt-10">
                <template v-slot:top>
                    <v-toolbar flat>
                        <h1 class="subheading grey--text">Persistent Volumes</h1>
...
<script>
    ...
    export default {
      ...
      created(){
        ...
        axios
            .get("http://192.168.1.64:8700/api/v1/persistentvolumes",options)
            .then(response => {
            console.log("response.data.items");
            console.log(response.data.items);
            this.persistentvolumes = response.data.items;
            this.loading = false
            })
            .catch(error => {
                console.log("Fail");
      ...
      methods:{
        ...
        deleteItem(item){
          ...
          axios.delete('http://192.168.1.64:8700/api/v1/persistentvolumes/'+item.metadata.name,body,options)
          .then(response => { 
            this.messageLogin = 'Persistent Volume eliminado com sucesso';
