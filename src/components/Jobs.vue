<template>
    <div>
        ...
        <v-container class="my-5 teste">
            <v-data-table :headers="headers" :items="jobs" :loading="loading" class="mt-10">
                <template v-slot:top>
                    <v-toolbar flat>
...
<script>
    ...
    export default {
      ...
      created(){
        ...
        axios
            .get("http://192.168.1.64:8700/apis/batch/v1/namespaces/default/jobs",options)
            .then(response => {
      ...
      methods:{
        ...
        deleteItem(item){
          ...
          axios.delete('http://192.168.1.64:8700/apis/batch/v1/namespaces/'+this.$store.state.nameSpace+'/jobs/'+item.metadata.name,options)
          .then(response => { 
            this.messageLogin = 'Job eliminado com sucesso';
