<template>
    <div>
        <v-container class="my-5 teste">
            <v-data-table :headers="headers" :items="daemonsets" :loading="loading" class="mt-10">
                <template v-slot:top>
                    <v-toolbar flat>
                        <h1 class="subheading grey--text">Deamon Sets</h1>
...
<script>
    ...
    export default {
      ...
      created(){
          ...
          axios
            .get("http://192.168.1.64:8700/apis/apps/v1/namespaces/"+this.$store.state.nameSpace+"/daemonsets",options)
            .then(response => {
            console.log(response.data.items);
            this.daemonsets = response.data.items;
            this.loading = false
            })
            .catch(error => {
                console.log("Fail");
      ...
      methods:{
        ...
        deleteItem(item){
          ...
          axios.delete('http://192.168.1.64:8700/apis/apps/v1/namespaces/dewfault/daemonsets/'+item.metadata.name,options)
          .then(response => { 
            this.messageLogin = 'Deamon Set eliminado com sucesso';
