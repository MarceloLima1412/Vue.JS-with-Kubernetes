<template>
    <div>
        ...
        <v-container class="my-5 teste">
            <v-data-table :headers="headers" :items="services" :loading="loading" class="mt-10">
                <template v-slot:top>
                    <v-toolbar flat>
                ...
                <template v-slot:item.props="{ item }">
                    <v-chip :color="getColor(item.spec)"></v-chip>
                </template>
    ...
    export default {
            ...
            for (const service of this.services){
              ...
              try{ 
                if(typeof (service.status.loadBalancer.ingress[0]) !== 'undefined')
                  Object.assign(service, {ipeporto: service.status.loadBalancer.ingress[0].ip+':'+service.spec.ports[0].port});
      ...
      methods:{
        ...
        deleteItem(item){
          ...
          axios.delete('http://192.168.1.64:8700/api/v1/namespaces/'+this.$store.state.nameSpace+'/services/'+item.metadata.name,options)
          .then(response => { 
            this.messageLogin = 'Service eliminado com sucesso';
