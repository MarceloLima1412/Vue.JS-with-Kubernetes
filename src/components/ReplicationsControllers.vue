<template>
    <div>
        ...
        <v-container class="my-5 teste">
            <v-data-table :headers="headers" :items="replicationscontrollers" :loading="loading" class="mt-10">
                <template v-slot:top>
                    <v-toolbar flat>
                        <h1 class="subheading grey--text">Replications Controllers</h1>
...
<script>
    ...
    export default {
      ...
      created(){
        ...
        axios
            .get("http://192.168.1.64:8700/api/v1/namespaces/"+this.$store.state.nameSpace+"/replicationcontrollers",options)
            .then(response => {
            console.log(response.data.items);
            this.replicationscontrollers = response.data.items;
            this.loading = false
            })
            .catch(error => {
                console.log("Fail");
      ...
      methods:{
        ...
        deleteItem(item){
          ...
          axios.delete('http://192.168.1.64:8700/api/v1/namespaces/'+this.$store.state.nameSpace+'/replicationcontrollers/'+item.metadata.name,body,options)
          .then(response => { 
            this.messageLogin = 'Replication Controller eliminado com sucesso';
